...
WHERE MADA IN	(SELECT P.MADA
				FROM NHANVIEN N, PHANCONG P
				WHERE N.MANV = P.MA_NVIEN AND N.HONV = N'Nguyễn'
				GROUP BY P.MADA)
OR MADA IN	(SELECT D.MADA
				FROM NHANVIEN N, PHONGBAN P, DEAN D
				WHERE P.TRPHG = N.MANV AND P.MAPHG = D.PHONG AND N.HONV = N'Nguyễn')
...
SELECT N.HONV, N.TENLOT, N.TENNV, N.DCHI
...
GROUP BY N.HONV, N.TENLOT, N.TENNV, N.DCHI
